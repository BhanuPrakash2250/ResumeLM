...
import { getAIPlanState } from '../ai-access';
...
async function runTrackedAIRequest<T extends { object: unknown; usage?: LanguageModelUsage }>(
  input: {
    route: string;
    userId: string | null;
    isPro: boolean;
...
export async function tailorResumeToJob(
  ...
  const { isPro, userId: id } = await getAIPlanState();
  const selectedConfig = withTaskModel({ task: "jobTailoring", isPro, config });
  const start = Date.now();
  console.log(
    `[TAILOR][TRY] ${selectedConfig.model} | STEP: Tailoring resume content`
  );
  try {
...
export async function formatJobListing(jobListing: string, config?: AIConfig) {
  ...
  const start = Date.now();
  console.log(
    `[FORMAT][TRY] ${selectedConfig.model} | STEP: Analyzing job description → Formatting requirements`
  );
  try {
      const { object } = await runTrackedAIRequest({
